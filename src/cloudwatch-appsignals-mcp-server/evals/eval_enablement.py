...
class MetricsTracker:
    ...
    def get_metrics(self, expected_tools: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        metrics.update({
            ...
            "file_read_count": len([c for c in file_op_calls if c["tool_name"] == "read_file"]),
            "file_write_count": len([c for c in file_op_calls if c["tool_name"] == "write_file"])
        })

        if expected_tools:
            expected_tool_set = set(expected_tools)
            called_tool_names = set(c["tool_name"] for c in self.tool_calls)
            ...
            metrics.update({
                "hit_rate": len(called_expected) / len(expected_tool_set) if expected_tool_set else 0.0,
                "expected_tools_called": sorted(list(called_expected)),
                "missing_expected_tools": sorted(list(missing)),
                "unexpected_tools_called": sorted(list(unexpected)),
            })
...
async def validate_with_llm(
    ...
    rubric_items  = "\n".join([
        f"{i+1}. {criterion}"
        for i, criterion in enumerate(task["validation_rubric"])
    ])
...
Instructions:
For each criterion in the rubric, evaluate whether it is satisfied by the changes.
...
async def main():
    ...
    parser.add_argument(
        "--task",
        help="Run specific task by ID (default: run all)"
    )
    parser.add_argument(
        "--tasks-file",
        default="tasks/enablement_tasks.json",
